...
export default function SideRays({
  ...
  const rendererRef = useRef<{ gl: WebGLRenderingContext & { canvas: HTMLCanvasElement }; dpr: number; setSize: (w: number, h: number) => void; render: (opts: { scene: unknown }) => void } | null>(null);
  ...
  const initWebGL = useCallback(async () => {
    ...
    // Dynamically import ogl (avoid SSR issues)
...
void main() {
  ...
  gl_FragColor = color;
    ...
    const updateSize = () => {
      ...
      (uniforms.iResolution.value as number[]) = [w * rendererRef.current.dpr, h * rendererRef.current.dpr];
    };
